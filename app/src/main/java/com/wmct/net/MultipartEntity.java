...
public class MultipartEntity implements HttpEntity {
    ...
    public void addFilePart(final String key, final File file) {
        ...
        try {
            ...
            writeFirstBoundary();
            final String type = CONTENT_TYPE + TYPE_OCTET_STREAM +"; charset=\"UTF-8\""+ NEW_LINE_STR;
    ...
    public void addFilePart(final String key, final File file, long memberid) {
        InputStream fin = null;
        try {
            //StringBuffer sb = new StringBuffer();
            //sb.append(URLEncoder.encode("memberid", "UTF-8")).append('=').append(memberid);
            //mOutputStream.write(sb.toString().getBytes("UTF-8"));
            fin = new FileInputStream(file);
            writeFirstBoundary();
    ...
    private byte[] getContentDispositionBytes(String paramName, String fileName, long memberid) {
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append(CONTENT_DISPOSITION + "form-data; name=\"" + paramName + "\"");
        // 文本参数没有filename参数,设置为空即可
        if (!TextUtils.isEmpty(fileName)) {
